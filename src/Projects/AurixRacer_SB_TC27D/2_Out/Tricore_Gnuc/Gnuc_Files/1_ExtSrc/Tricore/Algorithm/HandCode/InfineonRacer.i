...
# 194 "../../_LibSrc/iLLD_1_0_1_8_0__TC27D/Src/BaseSw/iLLD/TC27D/Tricore/Gpt12/IncrEnc/IfxGpt12_IncrEnc.h"
typedef struct
{
    sint32 rawPosition;
    float32 speed;
...
# 258 "../../_LibSrc/iLLD_1_0_1_8_0__TC27D/Src/BaseSw/iLLD/TC27D/Tricore/Gpt12/IncrEnc/IfxGpt12_IncrEnc.h"
extern boolean IfxGpt12_IncrEnc_init(IfxGpt12_IncrEnc *driver, const IfxGpt12_IncrEnc_Config *config);
# 268 "../../_LibSrc/iLLD_1_0_1_8_0__TC27D/Src/BaseSw/iLLD/TC27D/Tricore/Gpt12/IncrEnc/IfxGpt12_IncrEnc.h"
extern void IfxGpt12_IncrEnc_initConfig(IfxGpt12_IncrEnc_Config *config, Ifx_GPT12 *gpt12);
# 283 "../../_LibSrc/iLLD_1_0_1_8_0__TC27D/Src/BaseSw/iLLD/TC27D/Tricore/Gpt12/IncrEnc/IfxGpt12_IncrEnc.h"
...
extern void IfxGpt12_IncrEnc_resetTurn(IfxGpt12_IncrEnc *driver);

extern void IfxGpt12_IncrEnc_updateFromT2_buff(IfxGpt12_IncrEnc *driver);
...
# 405 "../../_LibSrc/iLLD_1_0_1_8_0__TC27D/Src/BaseSw/iLLD/TC27D/Tricore/Gpt12/IncrEnc/IfxGpt12_IncrEnc.h"
...
# 33 "../../MyApp/AurixRacer/0_Src/AppSw/Tricore/SnsAct/BasicGpt12Enc.h"
typedef struct{
        float32 speed;
        float32 rawPosition;
        IfxStdIf_Pos_Dir direction;
...
float32 MIN_INDEX = 40 - 10;
float32 MAX_INDEX = 40 + 10;

float32 MIN_INDEX_RIGHT = 80 - 10;
float32 MAX_INDEX_RIGHT = 80 + 10;
...
void convolutionOP_RIGHT(void){
    ...
    for (n = 0; n < 128 + 3 - 1; n++)
    {
        ...
        for (k = kmin; k <= kmax; k++)
        {
         IR_LineData.Result[n] += IR_LineScan.adcBuffer[1][k] * IR_LineData.Transfer[n - k];
        }
    }
}
 ...
 for (uint32 i = (5 / 2); i < 128 - (5 / 2); i++) {
       for (uint32 j = -(5 / 2); j <= 5 / 2; j++) {
           IR_LineData.sample[j + (5 / 2)] = IR_LineScan.adcResult[0][i + j];
              IR_LineData.sample_RIGHT[j + (5 / 2)] = IR_LineScan.adcResult[1][i + j];
           if (j == 5 / 2) {
...
void median_filter_RIGHT(void) {
 for (uint32 i = (5 / 2); i < 128 - (5 / 2); i++) {
       for (uint32 j = -(5 / 2); j <= 5 / 2; j++) {
           IR_LineData.sample[j + (5 / 2)] = IR_LineScan.adcResult[1][i + j];
           if (j == 5 / 2) {
               for (uint32 m = 0; m < 5 - 1; m++) {
                   for (uint32 n = m + 1; n < 5; n++) {
                       if (IR_LineData.sample[m] < IR_LineData.sample[n]) {
...
boolean is_THRESHOLD(void){
    uint32 index = 0;
    float32 threshold = 400;

    for(index = 4; index < 128 - 4; index += 2){
...
boolean is_THRESHOLD_MIDDLE(void){
    ...
    for(index = 4; index < half_index; index += 2){
...
void threshold_LINE(void){
    uint32 index = 0;
    float32 threshold = 400, threshold_RIGHT = 1000;
...
boolean is_WIDE_LANE(void){
    if(IR_LineScan.adcResult[0][IR_LineData.present + 10] < 400){
        return 1;
    }
    else if(IR_LineScan.adcResult[0][IR_LineData.present - 10] < 400){
        return 1;
    }
    else
        return 0;
...
boolean is_WIDE_LANE_RIGHT(void){
    if(IR_LineScan.adcResult[1][IR_LineData.present + 10] < 400){
        return 1;
    }
    else if(IR_LineScan.adcResult[1][IR_LineData.present - 10] < 400){
        return 1;
    }
    else
        return 0;
...
void threshold_LINE_RIGHT(void){
    ...
    if(!IR_LineData.Direction_Determined_RIGHT){
     ...
     IR_LineData.Direction_Determined_RIGHT = 1;
    }
...
boolean IsInSchoolZone_THRESHOLD(void){
    uint32 index = 0;
    uint32 half_index = 128 / 2;
    float32 SCHOOLZONE_DETECTION = 400;
    uint32 line_count = 0;
...
boolean IsOutSchoolZone_THRESHOLD(void){
    ...
    for(index = half_index; index < 128 - 4; index += 2){
        if(IR_LineScan.adcBuffer[0][index] < SCHOOLZONE_DETECTION){
            line_count++;
            break;
        }
...
boolean IsInSchoolZone(void){
    ...
    uint32 SCHOOLZONE_DETECTION = MaxVal;

    for(index = IR_LineData.present + 4; index < 128 - 4; index++){
