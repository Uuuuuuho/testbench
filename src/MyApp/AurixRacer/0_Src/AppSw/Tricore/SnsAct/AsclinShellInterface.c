...
boolean AppShell_enc(pchar args, void *data, IfxStdIf_DPipe *io)
{
    ...
    {
    	IfxStdIf_DPipe_print(io, "  Encoder speed: %f, position: %f, direction: %1d, turn: %d "ENDL,
    			IR_getEncSpeed(), IR_getEncPosition(), IR_getEncDirection(), IR_getEncTurn());
    }
...
void AsclinShellInterface_runEncScan(void){
	IfxStdIf_DPipe_print(&g_AsclinShellInterface.stdIf.asc, "position: %5f, turn: %5d, speed: %5f" ENDL, IR_getEncPosition(), IR_getEncTurn(), IR_getEncSpeed());
}
